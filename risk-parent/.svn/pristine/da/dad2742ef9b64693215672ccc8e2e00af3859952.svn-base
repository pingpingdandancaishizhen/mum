    ...
    <section class="content content-padding">
					...
					<div class="input-group">
						...
						<select name="productId" class="form-control">
							...
							<c:forEach items="${productList}" var="p">
								<option value="${p.productType}">${p.productName}</option>
							</c:forEach>
		...
		<table class="table  table-striped table-hover table-responsive" id="workflow_table" data-toggle="table"
			...
			<tr>
				...
				<th data-field="productType">产品类型</th>
...
function operateFormatter(value, row, index) {
	...
	return '<div class="btn-group-edit" >'+[row1,row3].join(' ')+'</div>';
}
 </script>
<script type="text/javascript" src="${ctx}/static/pagejs/solution/p2pworkflow/workflow.js${timeStyle}"></script>
<shiro:hasPermission name="workflow:assign">
<script type="text/javascript" src="${ctx}/static/pagejs/solution/p2pworkflow/workflow-assign.js${timeStyle}"></script>
</shiro:hasPermission>
<shiro:hasPermission name="workflow:form">
<script type="text/javascript" src="${ctx}/static/pagejs/solution/p2pworkflow/workflow-form.js${timeStyle}"></script>
</shiro:hasPermission>
<shiro:hasPermission name="workflow:deploy">
<script type="text/javascript" src="${ctx}/static/pagejs/solution/p2pworkflow/workflow-deploy.js${timeStyle}"></script>
</shiro:hasPermission>
</body>
</html>