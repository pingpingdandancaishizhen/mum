...
$(function(){
	...
	// 注册验证
	var $form=$('#modifyCustomerForm').find('form');
	$form.validate({
		type:{
			validators: {
				notEmpty: {
			...
			validators: {
				...
				callback: {
					...
					callback: function(value, validator) {
						...
						if(flag){
							$("#gender").val(chooseGender(value)).trigger('change');
							$("#age").val(chooseAge(value));
							$("#birthday").val(chooseBirthday(value));
						}
						return flag;
					}
				},
	...
	$("input[name=jobType]").bind("change",function(){
		...
		if(jobType == "1"){
			...
			$("#jobTypeDiv").children("div[for*='SXRS']").each(function(){
				...
				$(this).find("select").removeAttr("disabled").val("");
	...
	var defaultIndustryJob=$("#industry").data('industryJob');
	$("#industry").positionSelect({
		...
		nameId: "job",
		defaultValue:defaultIndustryJob
	...
	worf.ajax({
		...
		success : function(data) {
			$('#modifyCustomerForm #registAddr').citypicker({simple:true,data:data.data});
			$('#modifyCustomerForm #liveAddr').citypicker({simple:true,data:data.data});
			$('#modifyCustomerForm #companyAddr').citypicker({simple:true,data:data.data});
		}
	});
	...
	$form.off().on('success.form.bv', function(e) {//点击提交之后
        ...
        //提交服务器
		var $modifyCustomerBtn=$('#modifyCustomerBtn');
		$modifyCustomerBtn.button('loading');
		// 获取表单提交的数据
		var submitData=$form.serializeArray();
		$.each(submitData,function (key, item) {
			var name=item.name;
			var type=$('[name="'+name+'"]').data('formatAccount');
			if(type){
				var value=item.value;
				item.value=tools.unFormatAccount(value);
			}
		});
		worf.ajax({
	...
	$("#modifyCustomerForm #modifyCustomerBtn").off().on('click',function(){
