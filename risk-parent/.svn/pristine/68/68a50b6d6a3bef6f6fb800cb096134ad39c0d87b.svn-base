...
import cn.sunfit.risk.buz.server.dao.metadata.BPMetaNodeDeptRelDAO;
...
public class CorpDeptServiceImpl implements CorpDeptService {
    ...
    @Autowired
    BPMetaNodeDeptRelDAO bpMetaNodeDeptRelDAO;

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void addDept(CorpDeptAddReq dept) {
        ...
        bpMetaNodeDeptRelDAO.deleteByDeptId(dept.getDomain(), dept.getId());
        List<BPMetaNodeDeptRel> nodes = dept.getNode();
        if (nodes != null) {
            for (BPMetaNodeDeptRel bpMetaNodeDeptRel : nodes) {
                bpMetaNodeDeptRel.setId(IdUtil.geneId());
                bpMetaNodeDeptRel.setDomain(dept.getDomain());
                bpMetaNodeDeptRel.setDeptId(dept.getId());
                bpMetaNodeDeptRel.setCreateUserId(dept.getUserId());
            }
            bpMetaNodeDeptRelDAO.batchInsert(dept.getDomain(), nodes);
        }
    }
    ...
    @Override
    public List<BPMetaNodeDeptRel> queryDeptMeteNodes(String domain, String deptId) {
        List<BPMetaNodeDeptRel> nodes = bpMetaNodeDeptRelDAO.selectByDeptId(domain, deptId);
        return nodes;
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateDept(CorpDeptModifyReq dept) {
        CorpDept oldDept = corpDeptDAO.selectByPrimaryKey(dept.getId());
