			...
			<div class="modal-body">
				<form onsubmit="return false;" class="custom-form form-horizontal"
					...
					<div class="panel-body">
					     ...
					     <label for="productCode" class="col-sm-2 control-label">
									产品：</label>
									<div class="col-sm-10">
										<select  class="form-control" id="productCode_imp" name="productCode">
										<c:forEach items="${products}" var="p">
											<option value="${p.productCode}">${p.productName}(${p.productCode})</option>
										</c:forEach>
										</select>
									</div>
					     </div>
		...
		$("#batch_import_btn").bind("click",function(){
				...
				domArr.forEach(function (v) {
					...
					uploader.option('formData', {
						productCode: $("#productCode_imp").val()
					});
							...
							if(res.data.success){
								if(res.data.fails.length > 0){
									...
									for(var i = 0;i<res.data.fails.length;i++){
										err += "第"+res.data.fails[i].line+"行,订单号："+res.data.fails[i].loanId+",失败原因:"+res.data.fails[i].errorInfo+" <br/>";
									}
								}
								if(res.data.fails.length == 0){
									$.alert("真棒！您已成功导入全部"+res.data.successCount+"条数据。");
								}else{
									$.alert("导入成功"+res.data.successCount+"条数据，失败"+res.data.errCount+"条数据！ <br/>"+err);
								}
							}else{
