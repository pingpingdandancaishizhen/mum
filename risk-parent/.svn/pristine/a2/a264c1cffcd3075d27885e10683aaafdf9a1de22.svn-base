...
import cn.sunfit.risk.buz.api.constants.form.BpStatus;
...
import cn.sunfit.risk.buz.api.vo.p2p.activiti.Attr;
...
import cn.sunfit.risk.buz.server.dao.p2p.activiti.BPBigAttrDAO;
import cn.sunfit.risk.buz.server.dao.p2p.activiti.BPDAO;
import cn.sunfit.risk.buz.server.dao.p2p.activiti.BPMetaDAO;
import cn.sunfit.risk.buz.server.dao.p2p.activiti.BPMetaFieldDAO;
...
import cn.sunfit.risk.buz.server.service.p2p.product.form.ProductFormFactory;
import cn.sunfit.risk.buz.server.util.p2p.CmdUtil;
import cn.sunfit.risk.buz.server.util.p2p.OperLogUtil;
import cn.sunfit.risk.buz.server.util.p2p.SolutionUtil;

@Service("risk.p2p.superFormService")
public class SuperFormServiceImpl implements SuperFormService {
    ...
    @Autowired
    private P2PProductDAO productDAO;
    ...
    @Override
    public void editFormData(FormQuery req, Map<String, String> formdata) throws ServiceException {
        // 提交任务
        List<ModifyField> modifyFields = null;
        BP bp = bpdao.findById(req.getCorpId(), req.getDomain(), req.getBpId());
        req.setLoanInfoId(bp.getLoanInfoId());
        modifyFields = this.modifyBpsLoanAttr(req, formdata, bp);
        BPOperLog log = OperLogUtil.createEditLog(req, bp, formdata, modifyFields);
        operLogService.insert(log);
    }

    @Override
    public void editFormDataDraft(FormQuery req, Map<String, String> formdata) {
        ...
        // 记录日志
        BPOperLog log = OperLogUtil.createDraftLog(req, bp, formdata, modifys);
        operLogService.insert(log);
    }
    ...
    private BPMetaForm getMetaForm(FormQuery req, Task task) {
        ...
        if (FormModelType.isHandle(req.getModel())) {
            if (StringUtils.isBlank(req.getTaskId())) {
                form = bpMetaFormDAO.selectStartFormByBpDefId(req);
            } else {
                form = bpMetaFormDAO.selectTaskFormByBpDefIdTaskKey(req, task.getTaskDefinitionKey());
            }
        } else if (FormModelType.isView(req.getModel())) {
            form = bpMetaFormDAO.selectViewFormByBpDefId(req);
        } else if (FormModelType.isEdit(req.getModel())) {
            form = bpMetaFormDAO.selectEditFormByBpDefId(req);
        } else {
            throw new ServiceException("获取表单配置失败");
        }
        if (form == null) {
            throw new ServiceException("获取表单配置失败");
        }
    ...
    private List<String> getReasons(FormQuery req, Task task) {
        List<String> reasons;
        if (StringUtils.isBlank(req.getTaskId())) {
            reasons = bpMetaNodeDAO.selectStartReasons(req.getDomain(), req.getBpDefId());
        } else {
            reasons = bpMetaNodeDAO.selectNodeReasons(req.getDomain(), req.getBpDefId(), task.getTaskDefinitionKey());
        }
        return reasons;
    }
    ...
    private Map<String, String> getSubmitFormData(Map<String, String> formdata) {
        ...
        submitformdata.put("to_node", formdata.get("to_node") == null ? "" : formdata.get("to_node"));
        return submitformdata;
    }

    private boolean isEnd(FormQuery req, BP bp, BPMetaOperation operationshas) {
        ...
        if (rpi == null) {
            ...
            if (hai == null || hai.isEmpty()) {
                ...
                // BPMetaNode node = bpMetaNodeDAO.selectByNodeKey(hai.get(0).getActivityId(), req.getBpDefId(),
                // req.getDomain());
                // productFormFactory.getHandler(req.getProductType()).updateStatus(req, node.getStatus());
                return true;
            }
        }
        return false;
    ...
    private List<ModifyField> modifyBpsLoanAttr(FormQuery req, Map<String, String> formdata, BP bp) {

        List<ModifyField> modifys = new ArrayList<ModifyField>();
        bp.setDomain(req.getDomain());
        // 把值更新到横表
        productFormFactory.getHandler(req.getProductType()).updateLoan(req, formdata);
        // 更新attr
        List<BPMetaField> fileds = bpMetaFieldDAO.findFieldKey(req);
        // 修改attr的日志值
    ...
    public FormData queryNodeFormData(FormQuery req) throws ServiceException {
        ...
        // 获取REASONS
        fd.setReasons(getReasons(req, task));
        return fd;
    }

    @Override
    public FormData queryNodeFormData4Edit(FormQuery req) {
        FormData fd = new FormData();
    ...
    private BP saveBpsLoanAttr(FormQuery req, Map<String, String> formdata) {
        ...
        bp.setProductType(req.getProductType());
        bp.setUpdateTime(new Date());
        bp.setDomain(req.getDomain());
        bp.setLoanInfoId(req.getLoanInfoId());
        bp.setPreOperTime(new Date());
        bp.setPreOperUserId(req.getUserId());
    ...
    private void saveTaskDraft(FormQuery req, Map<String, String> formdata) throws ServiceException {
        ...
        if (task == null) {
            ...
            this.editFormDataDraft(req, formdata);
        }
    }
    ...
    private BP submitStartForm(FormQuery req, Map<String, String> formdata, ProcessStartCmd cmd, boolean draft) {
        ...
        boolean isend = isEnd(req, bp, operationshas);
        bp.setDomain(req.getDomain());
        bpdao.update(bp);
        // 日志再看 XXX发起了流程
