...
<%@ include file="/WEB-INF/import/loading.jsp"%>
  ...
  <div class="content-wrapper">
    ...
    <section class="content content-padding">
            ...
            <form onsubmit="return false;" id="form" class="form-inline form-label-auto" role="form">
                ...
                <div class="form-group col-md-3 row-left">
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="insuranceId" name="insuranceId" placeholder="输入借款性质搜索">
                    </div>
                </div>
                <div class="form-group col-md-3 row-left">
                    <div class="input-group">
                ...
                <div class="form-group col-md-3 row-left">
                    <div class="input-group">
                        <label class="label-head text-right">借款期限</label>
                ...
                <div class="form-group col-md-3 row-left">
                    <div class="input-group">
                        <label class="label-head text-right">已还期次</label>
                        <select  class="form-control" id="status" name="status">
                            <option value="">请选择</option>
                ...
                <div class="form-group col-md-3 row-left">
                    <div class="input-group">
                        <label class="label-head text-right">还款日</label>
                        <select  class="form-control" id="status" name="status">
                            <option value="">请选择</option>
        ...
        <div id="toolbar">
            <div class="btn-group">
                <shiro:hasPermission name="repayment:entering">
                    <div class="btn btn-white" id="btn_export">
                        <span>还款录入</span>
                    </div>
                </shiro:hasPermission>
                <shiro:hasPermission name="repayment:overdue">
                    <div class="btn btn-white" id="btn_overdue">
                        <span>逾期退保</span>
                    </div>
                </shiro:hasPermission>
                <shiro:hasPermission name="repayment:detail">
        ...
        <table class="table  table-striped table-hover" id="all_installment_table" data-toggle="table" style="width:3000px"
            ...
            <tr>
                ...
                <th data-field="totalPrice">借款金额</th>
                <th data-field="payMethod">还款方式</th>
                <th data-field="loanPeriodsNum">借款期限</th>
                <th data-field="loanProperty">借款性质</th>
                <th data-field="insurerName">保险公司</th>
                <th data-field="amount">已还期次</th>
                <th data-field="payMethod">已还本金</th>
                <th data-field="status">未还本金</th>
                <th data-field="orderSource">还款日</th>
                <th data-field="issuePlat">逾期天数</th>
                <th data-field="issueStatus">还款状态</th>
            </tr>
            </thead>
        </table>
      <!-- /.row -->
	...
	$('#btn_detail').bind("click",function(){
        ...
        if($table.find("input:checked").length === 0 ){
        	...
        	if(selected.length==0)
        		tools.addParentTab("${ctx}/repayment/queryRepaymentDetail?insuranceId="+$table.find("input:checked").parent().next().html(),$table.find("input:checked").parent().next().next().html()+"-查看信息");
        	else
        		tools.addParentTab("${ctx}/repayment/queryRepaymentDetail?insuranceId="+selected[0]['insuranceId'],$table.find("input:checked").parent().next().next().html()+"-查看信息");
        }
    });
	
	$('#btn_overdue').bind("click",function(){
		var selected = $table.bootstrapTable('getSelections');
        if($table.find("input:checked").length === 0 ){
            $.alert("请选择要查看的订单");
        }else if($table.find("input:checked").length > 1 ){
            ...
            if(selected.length==0)
                tools.addParentTab("${ctx}/repayment/queryOverdue?insuranceId="+$table.find("input:checked").parent().next().html(),$table.find("input:checked").parent().next().next().html()+"-查看信息");
            else
                tools.addParentTab("${ctx}/repayment/queryOverdue?insuranceId="+selected[0]['insuranceId'],$table.find("input:checked").parent().next().next().html()+"-查看信息");
