...
//        if(customerId.equals("-1")){//无客户，自动创建一个客户
//        	String custLicenseNum = formdata.get("cust_license_num");
//        	if (!customerService.checkCustomerExist(null, user.getId(),custLicenseNum, user.getDomain())) {
//                return Resp.buildErrorResponse("用户身份证号已经存在！");
//            }
//            if (!blackListService.checkCustomerBlackList(custLicenseNum, user.getDomain())) {
//                return Resp.buildErrorResponse("用户身份证号已被添加黑名单！");
//            }
//            customerId = customerService.autoSaveCustomer(formdata, user);
