...
$(function(){
	...
	$form.validate({
		corpName:{
			validators: {
				...
				remote: {
					url: web_root+'/asset/checkCorpNameExist',//验证地址
					message: '企业名称已经存在',//提示消息
					delay :  2000,//每输入一个字符，就发ajax请求，服务器压力还是太大，设置2秒发送一次ajax（默认输入一个字符，提交一次，服务器压力太大）
					type: 'POST',//请求方式
					data: function(validator) {
						return {
							"corpName" : validator.getFieldValue("corpName"),
		...
		corpAbbreviation: {
			validators: {
				...
				remote: {
					url: web_root+'/asset/checkCorpAbbreviationExist',//验证地址
					message: '企业简称已经存在',//提示消息
					delay :  2000,//每输入一个字符，就发ajax请求，服务器压力还是太大，设置2秒发送一次ajax（默认输入一个字符，提交一次，服务器压力太大）
					type: 'POST',//请求方式
					data: function(validator) {
						return {
							"corpAbbreviation" : validator.getFieldValue("corpAbbreviation"),
							"id" : validator.getFieldValue("id")
						};
					}
				}
		...
		institutionCode: {
			validators: {
				...
				regexp:{
					...
					regexp :/(^[0-9A-Z]{8,8}-[0-9A-Z]{1,1}$)|^([0-9A-Z]{1,1}-[0-9A-Z]{8,8}$)/,
					message: '请输入有效的机构代码'
				},
				remote: {
					url: web_root+'/asset/checkInstitutionCodeExist',//验证地址
					message: '机构代码已经存在',//提示消息
					delay :  2000,//每输入一个字符，就发ajax请求，服务器压力还是太大，设置2秒发送一次ajax（默认输入一个字符，提交一次，服务器压力太大）
					type: 'POST',//请求方式
					data: function(validator) {
						return {
							"institutionCode" : validator.getFieldValue("institutionCode"),
							"id" : validator.getFieldValue("id")
						};
					}
				}
		...
		fax: {
			validators: {
				...
				regexp:{
					regexp :/((^0{0,1}1[0-9]{10}$)|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/,
					message: '传真号请输入最多12位数字和减号'
				}
			}
		},
			...
			validators: {
				callback: {
					...
					callback: function(value, validator) {
						...
						if($("input[name='acceptAccountBank']").val()=='' || 
								$("input[name='acceptAccountBankVal']").val() != $("#plugin-acceptAccountBank").val() ||
								$.trim($("#plugin-acceptAccountBank").val()) == ''){
							return false;
						}
						return true;
			...
			validators: {
				callback: {
					...
					callback: function(value, validator) {
						...
						if($("input[name='repaymentBank']").val()=='' || 
								$("input[name='repaymentBankVal']").val() != $("#plugin-repaymentBank").val() ||
								$.trim($("#plugin-repaymentBank").val()) == ''){
							return false;
						}
						return true;
	...
	//suggest插件
	 function initBanks() {
	        $("#plugin-acceptAccountBank").bsSuggest('init', {
	            ...
	            showHeader: false,
	            clearable: true,
	            effectiveFields: ["bankName"],
	            url: web_root+'/asset/allBank',
	            idField: "bankCode",
	            keyField: "bankName"
	        }).on('onDataRequestSuccess', function (e, result) {
	            console.log('onDataRequestSuccess: ', result);
	        }).on('onSetSelectValue', function (e, keyword, data) {
	        	var aa = $("#plugin-acceptAccountBank").attr("data-id");
		   		  $("input[name='acceptAccountBank']").val(aa);
		   		$("input[name='acceptAccountBankVal']").val($("#plugin-acceptAccountBank").val());
	        ...
	        $("#plugin-repaymentBank").bsSuggest('init', {
	            /*url: "/rest/sys/getuserlist?keyword=",
	            effectiveFields: ["userName", "email"],
	            searchFields: [ "shortAccount"],
	            effectiveFieldsAlias:{userName: "姓名"},*/
	            ...
	            console.log('onDataRequestSuccess: ', result);
	        	 ...
	        	 var aa = $("#plugin-repaymentBank").attr("data-id");
	   		  $("input[name='repaymentBank']").val(aa);
	   		$("input[name='repaymentBankVal']").val($("#plugin-repaymentBank").val());
	            console.log('onSetSelectValue: ', keyword, data);
	        }).on('onUnsetSelectValue', function () {
	            console.log('onUnsetSelectValue');
	 ...
	 /* $("#plugin-acceptAccountBank").blur(function(){
		  var aa = $(this).attr("data-id");
		  $("input[name='acceptAccountBank']").val(aa);
	  });
	  
	  $("#plugin-repaymentBank").blur(function(){
		  var aa = $(this).attr("data-id");
		  $("input[name='repaymentBank']").val(aa);		  
	  });*/
