...
(function() {
	...
	Entropizer.prototype._evaluateClass = function(charClass, password,classes) {
		var chars, i;
	...
	Entropizer.prototype.evaluate = function(password,min) {
		...
		//默认密码不够长度就给个最低
		if(password.length<min) return 0.01
		// Find the alphabet of the password (the union of all the classes it uses)
		for (i = 0; i < this.classes.length; i++) {
			alphabetSize += this._evaluateClass(this.classes[i], password,this.classes);
		}
