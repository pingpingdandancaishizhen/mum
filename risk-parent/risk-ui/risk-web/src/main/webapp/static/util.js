...
var tools = {
	...
	addParentTab:function (url, name) {
		if(window.parent.addParentTab){
			window.parent.addParentTab(url,name);
		}else{
	...
	//以空格 格式化 银行账号等账号
	formatAccount:function (value) {
		var arg=arguments[1];
		if(arg){
			return value.replace(' ','')
		}
		return value.replace(/\s/g,'').replace(/([0-9a-zA-Z]{4})(?=[0-9a-zA-Z])/g,"$1 ")
	},
	formatAmount :function (value) {
		...
		var rex = /\d{1,3}(?=(\d{3})+$)/g;
		return value.replace(/,/g,'').replace(/^(-?)(\d+)((\.\d+)?)$/, function (s, s1, s2, s3) {
