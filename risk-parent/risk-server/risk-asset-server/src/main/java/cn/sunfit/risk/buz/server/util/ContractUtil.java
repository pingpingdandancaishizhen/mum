...
public class ContractUtil {
     ...
     * @return 返回封装好的数据对象
     */
    public static List<ContractFillContent> createFillContentList(Map<String, ContractField> dataMap,
            List<ContractFieldRel> relList) {
        List<ContractFillContent> list = new ArrayList<ContractFillContent>();

        if (relList == null || dataMap == null) {
            return list;
        }
        ...
        for (ContractFieldRel rel : relList) {
            body = new ContractFillBody();
            if (dataMap.get(rel.getDataField()) != null) {
                val = dataMap.get(rel.getDataField()).getValue();
                body.setShowText(val == null ? "" : val);
                body.setValKey(rel.getTempField());
                body.setValType(dataMap.get(rel.getDataField()).getType());
                if (VAL_TYPE_TEXT.equals(body.getValType())) {
                    textList.add(body);
                } else if (VAL_TYPE_TAB.equals(body.getValType())) {
                    tableList.add(body);
                } else if (VAL_TYPE_IMG.equals(body.getValType())) {
                    imageList.add(body);
                }
            }
        }
    ...
    private static void inImgToPDF(PdfStamper stamper, ContractFillBody body) throws DocumentException,
        ...
        if (null != body.getShowText()) {
            Image img = (Image) body.getShowText();
            int percent = 100;
            while (percent > 0 && img.getScaledWidth() > 100) {
                img.scalePercent(percent--);
            }
            if (img != null) {
                img.setAbsolutePosition(position[1] - img.getScaledWidth() / 2, position[2] - img.getScaledHeight() / 2);
